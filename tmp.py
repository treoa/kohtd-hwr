...
class Train(object):
    ...
    def calc_loss(self):
        self.recon = tf.losses.absolute_difference(
            self.label, self.MODEL.output)
        self.KL = tf.reduce_mean(0.5 * tf.reduce_mean(tf.exp(self.EST.sigma) +
                                 tf.square(self.EST.mu) - 1. - self.EST.sigma, axis=(1, 2, 3)))

        self.AE_recon = tf.losses.absolute_difference(
            self.input, self.DEC.output)

        f_logit = self.DIS_fake.logit
        r_logit = self.DIS_real.logit

        self.d_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.zeros_like(f_logit),  logits=f_logit))
        self.d_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.ones_like(r_logit), logits=r_logit))
        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.ones_like(f_logit), logits=f_logit))
    ...
    def __call__(self):
        ...
        var_list = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope='Denoise')
        var_EST = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope='EST')
        var_DEC = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope='DEC')

        var_DIS = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope='DIS')

        self.global_step = tf.Variable(
            self.step, name='global_step', trainable=False)

        '''Learning rate and the decay rules'''
        self.learning_rate = tf.train.exponential_decay(
            self.learning_rate, self.global_step, 100000, 0.5, staircase=True)
        ...
        self.opt = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(
            self.loss, global_step=self.global_step, var_list=var_list+var_EST+var_DEC)
        self.d_opt = tf.train.AdamOptimizer(
            learning_rate=self.learning_rate, beta1=0.5).minimize(self.d_loss, var_list=var_DIS)
        ...
        self.summary_op = tf.summary.merge([tf.summary.scalar('loss', self.loss),
            ...
            tf.summary.scalar(
            'AE_recon', self.AE_recon),
            tf.summary.scalar(
            'G_loss', self.g_loss),
            tf.summary.scalar(
            'D_loss', self.d_loss),
            tf.summary.image('1.Input', tf.clip_by_value(
                self.input, 0., 1.), max_outputs=4),
            tf.summary.image('2_1.output', tf.clip_by_value(
                self.MODEL.output, 0., 1.), max_outputs=4),
            tf.summary.image('2_2.AErecon', tf.clip_by_value(
                self.DEC.output, 0., 1.), max_outputs=4),
            tf.summary.image(
            '3.GT', self.label, max_outputs=4)
        ])
        ...
        with tf.Session(config=self.conf) as sess:
            ...
            could_load, model_step = load(
                self.saver, sess, self.checkpoint_dir, folder='Model%d' % self.trial)
            if could_load:
            ...
            while True:
                try:
                    ...
                    if step % 1000 == 0:
                        ...
                        loss_, recon_, KL_, AE_recon_, g_loss_, d_loss_, summary, LR_ = sess.run(
                            [self.loss, self.recon, self.KL, self.AE_recon, self.g_loss, self.d_loss, self.summary_op, self.learning_rate])
                        print('Iteration:', step, 'Loss: %.6f = %.6f + %.6f + %.6f + %.6f' %
                              (loss_, recon_, KL_, AE_recon_, g_loss_), 'd_loss: %.6f' % d_loss_)
                    ...
                    if step % 10000 == 0:
                        save(self.saver, sess,
                             self.checkpoint_dir, self.trial, step)
    ...
    def augment(self, labels, images,
        ...
        with tf.name_scope('augmentation'):
            ...
            if rotate:
                angles = tf.to_float(tf.random_uniform(
                    [batch_size], 0, 4, dtype=tf.int32))
    ...
    def _parse_function(self, example_proto):
        keys_to_features = {'label': tf.FixedLenFeature(
            [], tf.string), 'image': tf.FixedLenFeature([], tf.string)}

        parsed_features = tf.parse_single_example(
            example_proto, keys_to_features)

        label = parsed_features['label']
        label = tf.divide(tf.cast(tf.decode_raw(
            label, tf.uint8), tf.float32), 255.)
        label = tf.reshape(label, [self.HEIGHT, self.WIDTH, self.CHANNEL])

        img = parsed_features['image']
        img = tf.divide(tf.cast(tf.decode_raw(
            img, tf.uint8), tf.float32), 255.)
